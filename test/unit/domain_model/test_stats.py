...
def test_init(stats):
    ...
    assert stats.watched_actors == {Actor("Will Smith"): 2, Actor("Daniel Radcliff"): 1, Actor("Selena Gomez"):1, Actor("Tom Cruise"): 2}
...
def test_update(stats):
    ...
    assert stats.watched_directors == {Director("Peter Jackson"): 2, Director("Taika Waititi"): 1, Director("Christopher Nolan"): 1}
    assert stats.watched_genres == {Genre("Comedy"): 3, Genre("Romance"): 1, Genre("Drama"): 1, Genre("Action"): 2, Genre("Sci-Fi"): 1}
...
def test_top_actors(stats):
    ...
    assert stats.top_actors(-1) == [Actor("Tom Cruise"), Actor("Will Smith"), Actor("Selena Gomez"), Actor("Daniel Radcliff")]
